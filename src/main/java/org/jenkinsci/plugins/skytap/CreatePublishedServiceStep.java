...
public class CreatePublishedServiceStep extends SkytapAction {
	...
	public Boolean executeStep(AbstractBuild build,
		...
		try {
			runtimeConfigurationID = SkytapUtils.getRuntimeId(configurationID, configurationFile);
		...
		// build request
		HttpGet hg = SkytapUtils.buildHttpGetRequest(requestURL, authCredentials);
		...
		String createServiceURL = buildCreatePublishedServiceURL(interfaceId, portNumber);
		
		// build request
		HttpPost hp = SkytapUtils.buildHttpPostRequest(createServiceURL, authCredentials);
		...
		JenkinsLogger.log("New service published on interface " + interfaceId + ", port " + this.portNumber);
	...
	/**
	 * Takes the json array returned and searches for the interface
	 * element matching the network name provided by the user.
	 * 
	...
	private String getInterfaceId(String httpResponse, String netName) throws SkytapException{
		...
		JenkinsLogger.log("Searching configuration's interfaces for interface with network name: "
				+ netName);
		...
		// if no interface id was returned, throw an exception that will fail the build  step
	...
	private Boolean preFlightSanityChecks(){
		...
		// make sure network name, port number and published service file were provided
